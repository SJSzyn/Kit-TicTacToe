...
if (isTesting) {
  let testDay = prompt("day");
  let testHour = prompt("hr");
  dayOfWeek = parseInt(testDay);
  endOfWeekdayHour = parseInt(testHour);
}

if ((dayOfWeek >= 1 && dayOfWeek <= 5) || (dayOfWeek === 5 && endOfWeekdayHour < 17) || (dayOfWeek === 1 && endOfWeekdayHour > 8)) {
  console.log(weekdayMsg);
} else {
  console.log(weekendMsg);
}